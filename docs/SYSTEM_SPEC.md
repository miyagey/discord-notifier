...
| :--- | :--- | :--- | :--- | :--- |
| **`form/コード.js`** | `onFormSubmit` | **フォーム** | **フォーム送信時** | 1. フォームの回答からイベント/申込行を追記<br>2. ID自動採番 & IFS数式を挿入<br>3. 新規イベント時は Google カレンダーへリアルタイム自動登録 & Discord通知<br>4. フォーム選択肢を更新<br>5. Discord に新着申込を通知 |
| **`form/コード.js`** | `updateFormOptions` | 関数呼出 | フォーム送信時 | 終了日が今日以降のイベントを取得し、フォームの選択肢（ドロップダウン）を動的に再構築 |
| **`spreadsheet/`** | `remindEndDate` | 時間主導型 | 毎日 (午前) | 1. 本日申込締切の通常チケットを Discord へ通知（フォームリンク付き）<br>2. 翌日開始の**先着受付**がある場合は2通目として別途通知<br>3. 受付期間中の**リセール**がある場合は3通目として別途通知 |
...
| **`spreadsheet/`** | `registerEventsToCalendar` | 手動 / 時間主導型 | 必要に応じて実行 | スプレッドシート上の未登録イベントを一括で Google カレンダーに登録し、H列に CalID を書き戻す（バックアップ用） |
