...
| 設問タイトル | フォーム要素 | 必須 | 概要・データ形式 |
| :--- | :--- | :--- | :--- |
| **受付名（先行/一般など）** | 記述式 (ショート) | 必須 | 受付区分 (例: `アソビストア限定先行`, `一般先着`) |
| **申込URL** | 記述式 (ショート) | 任意 | チケット申し込みページの URL |
...
```excel
=IFS(
  AND(NOT(ISBLANK(F2)), TODAY() < INT(F2)), "開始前",
  AND(NOT(ISBLANK(G2)), TODAY() <= INT(G2)), "受付期間",
  AND(NOT(ISBLANK(I2)), TODAY() < INT(I2)), "抽選終了・当落確認前",
  AND(NOT(ISBLANK(J2)), TODAY() <= INT(J2)), "当落確認・入金期間",
  TRUE, "期間終了"
)
```
